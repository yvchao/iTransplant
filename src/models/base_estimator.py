...
class NeuralEstimator(Estimator):
    ...
    def fit(self, X, y, max_iter=10, batch_size=256, learning_rate=1e-3, retain_graph=False, tolerance=3, validation_split=None,verbose=False):
        ...
        if validation_split is not None:
            sss = StratifiedShuffleSplit(n_splits=1, test_size=validation_split, random_state=random_state)
        ...
        for epoch in tbar:
            ...
            if no_improvement_count>=tolerance:
                if verbose:
                    print(f'loss does not improve in {tolerance} epoches, stop training at epoch {epoch}, current loss: {validation_loss}')
