...
def Actor_Loss(data_in,net_out,lambda_=0.1, eps=1e-8):
    ...
    w=data_in['sample_weight'].view(-1,1)
    critic_loss=F.binary_cross_entropy(a_critic,a,weight=w,reduction='sum').detach()
    baseline_loss=F.binary_cross_entropy(a_baseline,a,weight=w,reduction='sum').detach()
    Reward=critic_loss - baseline_loss
    p_selection=net_out['p_selection']
    selection=net_out['selection']
    actor_loss=Reward*torch.sum(selection*torch.log(p_selection+eps)+(1-selection)*torch.log(1-p_selection+eps),dim=1)+lambda_ * torch.mean(p_selection, dim = 1)
