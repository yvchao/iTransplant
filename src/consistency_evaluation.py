...
numerical_features = [
    'AGE', 'HGT_CM_CALC', 'WGT_KG_CALC', 'BMI_CALC', 'CREAT_TX', 'INR_TX',
    'TBILI_TX', 'MELD_PELD_LAB_SCORE', 'FINAL_SERUM_SODIUM', 'ALBUMIN_TX',
    'NUM_PREV_TX', 'AGE_DON', 'HGT_CM_DON_CALC', 'WGT_KG_DON_CALC',
    'TBILI_DON', 'CREAT_DON', 'SGOT_DON', 'SGPT_DON'
]


def perturb_samples(X_train,
                    X_test,
                    perturbed_features,
                    perturb_signs,
                    data_dict,
    ...
    for f, perturb_sign in zip(perturbed_features, perturb_signs):
        ...
        if f in patient_features:
            ...
            if is_num:
                std = np.std(X_tr[:, idx])
                X_purterb[:,
                          idx] = X_purterb[:, idx] + perturb_sign * scale * std
            else:
                X_purterb[:, idx] = 1 - X_purterb[:, idx]
            if f in [
                    'CREAT_TX', 'INR_TX', 'TBILI_TX', 'FINAL_SERUM_SODIUM',
                    'DIAL_TX_1', 'DIAL_TX_0'
            ]:
                ...
                MELD_old = meld_na(pd.DataFrame(X_te,
            ...
            if is_num:
                std = np.std(O_tr[:, idx])
                O_purterb[:,
                          idx] = O_purterb[:, idx] + perturb_sign * scale * std
            else:
...
def single_feature_consistency(feature, X_train, X_test, data_dict, estimator,
                               preprocessor):
    consistency_masks = []
    for perturb_sign in [1.0, -1.0]:
        perturbed_features = [feature]
        perturb_signs = [perturb_sign]
        perturbed_samples, x_te = perturb_samples(X_train,
                                                  X_test,
                                                  perturbed_features,
                                                  perturb_signs,
                                                  data_dict,
        ...
        prob_perturb = estimator.predict_proba(
            preprocessor.transform(perturbed_samples))[:, 1]
        expected_change = prob_cf - prob_base
        real_change = prob_perturb - prob_base
        consistency_mask = (real_change == 0) * (
            np.abs(expected_change) < 0.01) + (real_change != 0) * (
...
def single_feature_consistency_LIME(feature, X_train, X_test, data_dict,
                                    column_dict, estimator, preprocessor):
    ...
    predict_fn = lambda x: estimator.predict_proba(
        preprocessor.transform(
            np.concatenate([x, np.zeros((len(x), c_dim))], axis=1)))
...
def consistency_evaluation(data_dir, center, report_dir, seed, max_iter=200):
    ...
    perturbed_features = [
        'DIAL_TX_1', 'CREAT_TX', 'INR_TX', 'TBILI_TX', 'FINAL_SERUM_SODIUM',
        'AGE_DON', 'SGOT_DON', 'SGPT_DON', 'HGT_CM_DON_CALC',
        'WGT_KG_DON_CALC', 'SHARE_TY_3', 'SHARE_TY_4', 'SHARE_TY_5',
        'HEP_C_ANTI_DON_1', 'HBV_CORE_DON_1', 'ETHCAT_DON_2', 'GENDER_DON_0',
        'deathcirc_1', 'NON_HRT_DON_1', 'macro_1', 'macro_2', 'micro_1',
        'micro_2'
    ]
    ...
    for lambda_ in [0.0, 0.01, 0.1]:
        ...
        for feature in perturbed_features:
            consistency = single_feature_consistency(feature, X_train, X_test,
                                                     data_dict, estimator,
                                                     preprocessor)
    ...
    mean_consistency = df.loc[:, considered_patient_features].mean(
        axis=1) * 0.5 + df.loc[:, considered_donor_features].mean(axis=1) * 0.5
    ...
    clf, [preprocessor,
          estimator] = create_estimator(INVASEEstimator,
                                        data_dict,
                                        column_dict,
                                        random_state=seed,
                                        hyper_parameter=hyper_parameter)
    ...
    if len(reports) == 1 and estimator.name in reports[0]['model'].unique():
        ...
        for feature in perturbed_features:
            consistency = single_feature_consistency_LIME(
                feature, X_train, X_test, data_dict, column_dict, estimator,
                preprocessor)
            consistencies.append(consistency)
        record = [f'{estimator.name}'] + consistencies
        df = pd.DataFrame([record], columns=['model'] + perturbed_features)
        mean_consistency = df.loc[:, considered_patient_features].mean(
            axis=1) * 0.5 + df.loc[:, considered_donor_features].mean(
                axis=1) * 0.5
        df.loc[:, 'avg consistency'] = mean_consistency
        reports.append(df)
    df = pd.concat(reports)
    ...
    clf, [preprocessor,
          estimator] = create_estimator(BCEstimator,
                                        data_dict,
                                        column_dict,
                                        random_state=seed,
                                        hyper_parameter=hyper_parameter)
