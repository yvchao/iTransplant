...
def benchmark_one_center(model, data_dir, center,scoring,max_iter,random_state,hyper_parameter,validation_split, n_tests=10):
    random_state = check_random_state(random_state)
    seed_list=random_state.randint(1e8,size=(n_tests,))
    scores=[]
    for seed in seed_list:
        score=train_evaluate(model, data_dir, [center],seed,scoring,verbose=False,discretize=False, max_iter=max_iter,validation_split=validation_split,hyper_parameter=hyper_parameter)
        scores.append(score)
    stat=pd.DataFrame(scores)
    stat.loc[:,'center']=center
    return stat

def benchmark_multiple_centers(model, data_dir, centers,scoring,max_iter,random_state,hyper_parameter,validation_split=0.2, n_tests=10):
    stats=[]
    for center in centers:
        stat=benchmark_one_center(model, data_dir, center,scoring,max_iter,random_state,hyper_parameter,validation_split,n_tests)
        stats.append(stat)
    return pd.concat(stats)

def benchmark(data_dir, report_dir, centers, configurations, scoring, n_tests=5, max_iter=200, seed=19260817):
    np.random.seed(seed)
    ...
    for i, configuration in enumerate(tbar):
        ...
        if description in ['LOWESS']:
            record=benchmark_multiple_centers(model, data_dir, centers,scoring,max_iter,seed,hyper_parameters,validation_split,n_tests=1)
        else:
            record=benchmark_multiple_centers(model, data_dir, centers,scoring,max_iter,seed,hyper_parameters,validation_split, n_tests)
...
def summarize_benchmark(report_dir, return_tables=True):
    ...
    row_order=['Logistic Regression','Per-cluster Logistic Regression','Decision Tree','Per-cluster Decision Tree','LOWESS','LASSO','Random Forest','INVASE','BC','iTransplant']
...
def benchmark_visualization(benchmark_results, metric='AUC-PRC'):
    ...
    df=pd.pivot_table(benchmark_results, values=f'{metric}', index='center', columns=['description'], aggfunc={f'{metric}':np.mean})
...
def center_wise_summary(selected_records,center,metric):
    df=pd.pivot_table(selected_records, values=f'{metric}', index='center', columns=['description'], aggfunc={f'{metric}':np.mean})
    means=df.loc[center]

    df=pd.pivot_table(selected_records, values=f'{metric}', index='center', columns=['description'], aggfunc={f'{metric}':np.std})
    stds=df.loc[center]
