...
def get_saved_model_path(center, model_name, model_path= "saved_models"):
    ...
    path_tail = os.path.join(model_path,f'{center}-{model_name}-preprocessor.joblib')
    preprocessor_path = resource_filename("src", path_tail)
    model_name = f'{center}-{model_name}.pth'
    return model_name, preprocessor_path

def train_model(center, X_train, y_train, data_dict, column_dict ,max_iter, seed, hyper_parameters):
    clf,[preprocessor,estimator]=create_estimator(iTransplantEstimator,data_dict,column_dict,random_state=seed,hyper_parameter=hyper_parameters)

    clf.fit(X_train,y_train,estimator__validation_split=0.2,estimator__max_iter=max_iter, estimator__batch_size=400,estimator__verbose=True)

    model_name, preprocessor_path = get_saved_model_path(center,estimator.name)
    joblib.dump(preprocessor, preprocessor_path) 
    estimator.save_model(model_name)
    return estimator, preprocessor

def search_threshold(estimator,preprocessor, X_test,y_test, t_max=10.0,epsilon=1e-2, factor=0.05):
    ...
    while np.abs(t1-t0)>=epsilon:
        t_mid=0.5*(t0+t1)
        delta_t_mid=upper_bound-estimator.score(X_scaled,y_test,threshold=t_mid)
        if delta_t_mid<=0:
            t0=t_mid
        else:
            t1=t_mid
...
def population_level_decision_drivers(center, X_test, y_test, data_dict, column_dict,hyper_parameters):
    clf,[preprocessor,estimator]=create_estimator(iTransplantEstimator,data_dict,column_dict,random_state=None,hyper_parameter=hyper_parameters)

    model_name, preprocessor_path = get_saved_model_path(center,estimator.name)
    preprocessor = joblib.load(preprocessor_path) 
    estimator.load_model(model_name)

    threshold=search_threshold(estimator,preprocessor,X_test,y_test,t_max=1,factor=0.001)
    ...
    box=df.boxplot(column=list(reversed(label_criteria)),showfliers=False,ax=ax,return_type='dict',vert=False)
    xmin,xmax=ax.get_ylim()
    ax.plot([threshold,threshold],[xmin,xmax],'b--',alpha=0.5,zorder=-1)
    ax.plot([-threshold,-threshold],[xmin,xmax],'b--',alpha=0.5,zorder=-1)
    ax.set_xlim([-0.9,0.9])
    highlight_criteria=['Donor Age', 'MELD-Na','MELD', 'Donor Weight', 'Donor AST','National Donor', 'Local Donor', 'Regional Donor','HCV Positive Donor', 'HBV Positive Donor','Donation After Natural Death','Non-heart-beating Donation','High Degree MaS']
    for criterion in highlight_criteria:
        [idx],=np.where(criterion==np.array(list(reversed(label_criteria))))
...
def plot_decision_boundary(X,label,w0,w1,titles=['Logistic Regression','iTransplant']):
    ssss=2.5
    ...
    w1_policy=np.vectorize(lambda y1,y2: sigmoid(y1*r[0,1]+y2*r[1,1]))
    ...
    clf = svm.LinearSVC(C=1.3,class_weight='balanced',fit_intercept=True).fit(Y, label)
    w = clf.coef_[0]
    k = -w[0] / w[1]
    xx = np.linspace(-ssss, ssss)
    yy = k * xx - (clf.intercept_) / w[1]
    ax1.plot(xx,yy,'-.',linewidth=2,color='black',label='Optimal Decision Boundary')
    ax2.plot(xx,yy,'-.',linewidth=2,color='black',label='Optimal Decision Boundary')

    im1=ax1.contourf(y1mesh,y2mesh,w0_policy(y1mesh,y2mesh),cmap=cm,levels=10)
    mask=label==0
    ax1.scatter(Y[~mask,0], Y[~mask,1],c='cyan',marker='.',s=50,label='Accept',zorder=2)
    ax1.scatter(Y[mask,0], Y[mask,1],c='yellow',marker='x',label='Decline',zorder=1)
    
    ax1.legend()
    ax1.set_title(titles[0])
    
    im2=ax2.contourf(y1mesh,y2mesh,w1_policy(y1mesh,y2mesh),cmap=cm,levels=10)
    mask=label==0
    ax2.scatter(Y[~mask,0], Y[~mask,1],c='cyan',marker='.',s=50,label='Accept',zorder=2)
    ax2.scatter(Y[mask,0], Y[mask,1],c='yellow',marker='x',label='Decline',zorder=1)
...
def plot_decision_boundary_multi_clusters(W,X,c_label,clusters,a,n0,plot_logistic_regression=False):
    cm = plt.cm.get_cmap('coolwarm')
    ...
    for i,c in enumerate(clusters):
        ...
        if plot_logistic_regression:
            ...
            w_policy=np.vectorize(lambda y1,y2: sigmoid(y1*r[0,1]+y2*r[1,1]))

        c_value=a[loc]
        row=i//3
        col=i%3
        im=axs[col].contourf(y1mesh,y2mesh,w_policy(y1mesh,y2mesh),cmap=cm,levels=10)
        mask=c_value==0
        l1=axs[col].scatter(Y[~mask,0], Y[~mask,1],c='cyan',marker='.',s=50,label='Acceptance',zorder=2)
        l2=axs[col].scatter(Y[mask,0], Y[mask,1],c='yellow',marker='x',label='Rejection',zorder=1)
    ...
    fig.legend(handles, ['Accept', 'Decline'], bbox_to_anchor=(0.8, 0.95),loc = 'upper left')
    return fig,axs

def patient_specific_organ_preferences(center, X_train, y_train, X_test, y_test, data_dict, column_dict, hyper_parameters, seed):
    clf,[preprocessor,estimator]=create_estimator(iTransplantEstimator,data_dict,column_dict,random_state=seed,hyper_parameter=hyper_parameters)
    ...
    LR=LogisticRegression(penalty='none',random_state=seed,solver ='saga',class_weight='balanced',fit_intercept=False)
    LR_train=preprocessor.transform(X_train)[:,data_dict['X']['criteria']]
    LR.fit(LR_train,y_train)

    best_cluster_nums=find_best_cluster_num(W,Z,n_range=[2,10],random_state=seed)
    ...
    for c in range(n_clusters):
        ...
        in_cluster_apr = {'cluster':cluster_label, 'sample number': np.sum(mask), f'AUC-PRC ({estimator.name})': auc_prc, 'AUC-PRC (Logistic Regression)': auc_prc_LR}
    ...
    for c in range(n_clusters):
        mask=c_label==c
        cluster_label = f'cluster {c+1}'
        fig,ax=plot_polar_density(W_norm[mask]-np.mean(W_norm,axis=0),label_criteria.to_list(),baseline=True,y_lims=[-0.65,0.49])
        ...
        patient_features_dist['HCC negative (%)']=100*patient_features[mask,idx_hcc0].sum()/mask.sum()

        [idx_hcc1],=np.where(label_patient=='statushcc_1')
        patient_features_dist['HCC positive (%)']=100*patient_features[mask,idx_hcc1].sum()/mask.sum()

        [idx_meld],=np.where(label_patient=='MELD_PELD_LAB_SCORE')
        patient_features_dist['HCC positive & MELD <= 20 (%)']=100*((patient_features[mask,idx_hcc1])*(patient_features[mask,idx_meld]<=20)).sum()/mask.sum()

        for i in range(0,40,10):
            patient_features_dist[f'{i} <= MELD < {i+10} (%)']=100*((patient_features[mask,idx_meld]>=i)&(patient_features[mask,idx_meld]<i+10)).sum()/mask.sum()

        patient_features_dist['MELD >= 40 (%)']=100*((patient_features[mask,idx_meld]>=40)).sum()/mask.sum()
        cluster_wise_patient_features_dist.append(patient_features_dist)
    cluster_wise_patient_features_dist=pd.DataFrame(cluster_wise_patient_features_dist).set_index('cluster')

    ret_main ={}
    ret_appendix={}
    ret_appendix['cluster-wise apr improvement'] = cluster_wise_apr
    ret_main['cluster-wise policy signature distribution'] = cluster_wise_policy_signature_dist
    ret_main['cluster-wise patient feature distribution'] = cluster_wise_patient_features_dist

    cluster_wise_expert_selection={}
    for c in range(n_clusters):
        mask=c_label==c
        df=pd.DataFrame(data=gates[mask],columns=np.arange(1,1+estimator.num_experts))
    ...
    ret_appendix['cluster-wise expert selection'] = cluster_wise_expert_selection
    ...
    for c in considered_clusters:
        ...
        cluster_wise_decision_boundary_comparison_with_LR[f'cluster {c+1}'] = fig

    ret_appendix['decision boundary comparison with LR'] = cluster_wise_decision_boundary_comparison_with_LR
    
    fig,ax=plot_decision_boundary_multi_clusters(W,criteria,c_label,considered_clusters[:3],a,n0)
    ret_appendix['decision boundaries'] = fig
    return ret_main, ret_appendix

def cross_center_variation(center_A, center_B, X_test, y_test, data_dict, column_dict, hyper_parameters, seed=None, patient_idx=None):
    _,[preprocessor,estimator]=create_estimator(iTransplantEstimator,data_dict,column_dict,random_state=seed,hyper_parameter=hyper_parameters)

    model_name, preprocessor_path = get_saved_model_path(center_A,estimator.name)
    preprocessor = joblib.load(preprocessor_path) 
    estimator.load_model(model_name)

    _,[preprocessor1,estimator1]=create_estimator(iTransplantEstimator,data_dict,column_dict,random_state=seed,hyper_parameter=hyper_parameters)

    model_name, preprocessor_path = get_saved_model_path(center_B,estimator1.name)
    ...
    ret={}
    df =pd.DataFrame()
    df.loc[0,'difference in average organ offer acceptance rate'] =f'{np.abs(a_pred.mean()-a_pred1.mean()):.3f}'
    df.loc[0,'deviations of learned policies at individual level'] =f'{np.abs(a_pred-a_pred1).mean():.3f}±{np.abs(a_pred-a_pred1).std():.3f}'
    ...
    label2idx_p={'AGE':0,'MELD_PELD_LAB_SCORE':7,'INR_TX':5,'CREAT_TX':4,'TBILI_TX':6}
    label2idx_m={'SGOT_DON':5,'AGE_DON':2}
    label2idx_o={'SGOT_DON':5,'AGE_DON':0}

    candicates_indices, = np.where((np.abs(a_pred-a_pred1)>0.3)*(a==1)*(raw_patient_features[:,label2idx_p['MELD_PELD_LAB_SCORE']]>20)*(raw_patient_features[:,label2idx_p['AGE']]<60)*(raw_organ_features[:,label2idx_o['SGOT_DON']]>1)*(raw_organ_features[:,label2idx_o['SGOT_DON']]<3000))
    ...
    def p_accept1(AGE_DON=None,SGOT_DON=None):
        ...
        if SGOT_DON is not None:
            C[label2idx_m['SGOT_DON']]=SGOT_DON
        fake_sample=np.concatenate([X,O,C])[np.newaxis,:]
        p=estimator1.predict_proba(preprocessor1.transform(fake_sample))[0,1]
        return p.astype('float64')
    ...
    ax.plot(SGOT_DON,f1(SGOT_DON=SGOT_DON),'-.',color='green',label='Center B')
    ...
    ax.plot(AGE_DON,f1(AGE_DON=AGE_DON),'-.',color='green',label='Center B')
