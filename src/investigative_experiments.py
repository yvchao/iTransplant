...
def train_model(
    center, X_train, y_train, data_dict, column_dict, max_iter, seed, hyper_parameters
):
...
def search_threshold(
    estimator, preprocessor, X_test, y_test, t_max=10.0, epsilon=1e-2, factor=0.05
):
...
def population_level_decision_drivers(
    center, X_test, y_test, data_dict, column_dict, hyper_parameters
):
    clf, [preprocessor, estimator] = create_estimator(
        iTransplantEstimator,
        data_dict,
    ...
    threshold = search_threshold(
        estimator, preprocessor, X_test, y_test, t_max=1, factor=0.001
    )
    ...
    box = df.boxplot(
        column=list(reversed(label_criteria)),
        showfliers=False,
        ax=ax,
        return_type="dict",
        vert=False,
...
def plot_decision_boundary(
    X, label, w0, w1, titles=["Logistic Regression", "iTransplant"]
):
    ssss = 2.5
    ...
    clf = svm.LinearSVC(C=1.3, class_weight="balanced", fit_intercept=True).fit(
        Y, label
    )
    w = clf.coef_[0]
    k = -w[0] / w[1]
    ...
    ax1.plot(
        xx, yy, "-.", linewidth=2, color="black", label="Optimal Decision Boundary"
    )
    ax2.plot(
        xx, yy, "-.", linewidth=2, color="black", label="Optimal Decision Boundary"
    )
    ...
    mask = label == 0
    ax1.scatter(
        Y[~mask, 0], Y[~mask, 1], c="cyan", marker=".", s=50, label="Accept", zorder=2
    )
    ax1.scatter(
        Y[mask, 0], Y[mask, 1], c="yellow", marker="x", label="Decline", zorder=1
    )
...
def plot_decision_boundary_multi_clusters(
    W, X, c_label, clusters, a, n0, plot_logistic_regression=False
):
    cm = plt.cm.get_cmap("coolwarm")
    ...
    for i, c in enumerate(clusters):
        ...
        col = i % 3
        im = axs[col].contourf(
            y1mesh, y2mesh, w_policy(y1mesh, y2mesh), cmap=cm, levels=10
        )
        mask = c_value == 0
        l1 = axs[col].scatter(
            Y[~mask, 0],
        ...
        l2 = axs[col].scatter(
            Y[mask, 0], Y[mask, 1], c="yellow", marker="x", label="Rejection", zorder=1
        )
    ...
    fig.legend(
        handles, ["Accept", "Decline"], bbox_to_anchor=(0.8, 0.95), loc="upper left"
    )
    return fig, axs


def patient_specific_organ_preferences(
    center,
    X_train,
    y_train,
    X_test,
    ...
    for c in range(n_clusters):
        mask = c_label == c
        cluster_label = f"cluster {c+1}"
        fig, ax = plot_polar_density(
            W_norm[mask] - np.mean(W_norm, axis=0),
        ...
        patient_features_dist["HCC negative (%)"] = (
            100 * patient_features[mask, idx_hcc0].sum() / mask.sum()
        )

        ([idx_hcc1],) = np.where(label_patient == "statushcc_1")
        patient_features_dist["HCC positive (%)"] = (
            100 * patient_features[mask, idx_hcc1].sum() / mask.sum()
        )

        ([idx_meld],) = np.where(label_patient == "MELD_PELD_LAB_SCORE")
        patient_features_dist["HCC positive & MELD <= 20 (%)"] = (
            100
            * (
                (patient_features[mask, idx_hcc1])
                * (patient_features[mask, idx_meld] <= 20)
            ).sum()
            / mask.sum()
        )

        for i in range(0, 40, 10):
            patient_features_dist[f"{i} <= MELD < {i+10} (%)"] = (
                100
                * (
        ...
        patient_features_dist["MELD >= 40 (%)"] = (
            100 * ((patient_features[mask, idx_meld] >= 40)).sum() / mask.sum()
        )
    ...
    ret_main[
        "cluster-wise policy signature distribution"
    ] = cluster_wise_policy_signature_dist
    ret_main[
        "cluster-wise patient feature distribution"
    ] = cluster_wise_patient_features_dist
    ...
    for c in range(n_clusters):
        mask = c_label == c
        df = pd.DataFrame(
            data=gates[mask], columns=np.arange(1, 1 + estimator.num_experts)
        )
    ...
    ret_appendix[
        "decision boundary comparison with LR"
    ] = cluster_wise_decision_boundary_comparison_with_LR

    fig, ax = plot_decision_boundary_multi_clusters(
        W, criteria, c_label, considered_clusters[:3], a, n0
    )
    ret_appendix["decision boundaries"] = fig
    return ret_main, ret_appendix


def cross_center_variation(
    center_A,
    center_B,
    X_test,
    y_test,
    ...
    _, [preprocessor, estimator] = create_estimator(
        iTransplantEstimator,
        data_dict,
        column_dict,
        random_state=seed,
    ...
    _, [preprocessor1, estimator1] = create_estimator(
        iTransplantEstimator,
        data_dict,
        column_dict,
        random_state=seed,
    ...
    df = pd.DataFrame()
    df.loc[
        0, "difference in average organ offer acceptance rate"
    ] = f"{np.abs(a_pred.mean()-a_pred1.mean()):.3f}"
    df.loc[
        0, "deviations of learned policies at individual level"
    ] = f"{np.abs(a_pred-a_pred1).mean():.3f}±{np.abs(a_pred-a_pred1).std():.3f}"
