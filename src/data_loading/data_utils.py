...
def read_offer_data(data_dir, center,yr_begin=2002,yr_end=2021):
    ...
    offer_data=pd.read_hdf(data_loc,key='Offer',mode='r')[['OFFER_ACCEPT','MATCH_SUBMIT_DATE']]
    X=pd.read_hdf(data_loc,key='Patient',mode='r')
    O=pd.read_hdf(data_loc,key='Organ',mode='r')

    mask=((offer_data.MATCH_SUBMIT_DATE.dt.year>=yr_begin) & (offer_data.MATCH_SUBMIT_DATE.dt.year<yr_end)& (offer_data.OFFER_ACCEPT!='Z')).to_list()
...
def meld(X):
    ...
    MELD=np.round(9.57*np.log(np.clip(creat,1,1e10)) + 3.78* np.log(np.clip(X.TBILI_TX,1,1e10)) + 11.2 * np.log(np.clip(X.INR_TX,1,1e10)) + 6.43)
    return MELD
...
def create_criteria_discrete(X,O):
    ...
    step=5
    for i in range(0,35,step):
        df.loc[:,f'{i:d}≤MELD-Na<{i+step:d}']=1.0*(MELD_Na<(i+step))*(MELD_Na>=i)
    ...
    for i in range(15,80,step):
        df.loc[:,f'{i:d}≤Donor Age<{i+step:d}']=1.0*(age_don<(i+step))*(age_don>=i)
    ...
    for i in range(len(AST_range)-1):
        df.loc[:,f'{AST_range[i]}≤Donor AST<{AST_range[i+1]}']=1.0*(AST<AST_range[i+1])*(AST>=AST_range[i])
    df.loc[:,f'≤Donor AST≥500']=1.0*(AST>=500)

    ALT=O.SGPT_DON.values
    ALT_range=[50, 100,200,500]
    for i in range(len(ALT_range)-1):
        df.loc[:,f'{ALT_range[i]}≤Donor ALT<{ALT_range[i+1]}']=1.0*(ALT<ALT_range[i+1])*(ALT>=ALT_range[i])
...
def create_dataset(data_source,discretize=False):
    ...
    X=np.concatenate([patient_features.values,organ_features.values,criteria.values],axis=1)
