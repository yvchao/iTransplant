...
def create_estimator(model,
    ...
    for k, v in column_dict.items():
        transformers.append(
            (f'num_{k}', RobustScaler(quantile_range=(5, 95)), v[0]))
        transformers.append((f'cat_{k}', FunctionTransformer(), v[1]))
    preprocessor = ColumnTransformer(transformers=transformers)

    estimator = model(data_description=data_dict,
                      random_state=random_state,
                      **hyper_parameter)

    clf = Pipeline(steps=[('preprocessor',
                           preprocessor), ('estimator', estimator)])
