...
with open(match_records, 'rb') as dat_table:
    match_data = pd.read_table(dat_table,
                               names=columns,
                               na_values=[' ', '.'],
                               encoding='cp1252')

match = match_data.dropna(
    subset=['DONOR_ID', 'WLREG_AUDIT_ID_CODE', 'OFFER_ACCEPT'])

# only keep useful columns
selected_columns = [
    'DONOR_ID', 'WLREG_AUDIT_ID_CODE', 'LISTING_CTR_CODE', 'OFFER_ACCEPT',
    'ORGAN_PLACED', 'HOST_MATCH', 'OPO_ALLOC_AUDIT_ID', 'CLASS_ID',
    'PRIME_OPO_REFUSAL_ID', 'TXC_REFUSAL_CD', 'MATCH_SUBMIT_DATE'
]
...
print(selected.TXC_REFUSAL_CD.value_counts().head(10) /
...
for reason in reasons_to_keep:
    mask2 = (match.OFFER_ACCEPT == 'N') & (match.PRIME_OPO_REFUSAL_ID
                                           == reason)
    mask = mask | mask2
...
acceptance_rate = (hist_num /
                   hist_den)[hist_den > 10000].sort_values(ascending=False)
print(
...
columns = [
    'DONOR_ID', 'WLREG_AUDIT_ID_CODE', 'OFFER_ACCEPT', 'TXC_REFUSAL_CD',
    'MATCH_SUBMIT_DATE', 'CLASS_ID', 'OPO_ALLOC_AUDIT_ID'
]
