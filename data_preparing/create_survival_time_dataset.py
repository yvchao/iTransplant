...
from selected_features import PATIENT_FEATURES, INITIAL_PATIENT_FEATURES, ORGAN_FEATURES, COLUMN_IS_CATEGORICAL
...
EXTRA_COLUMNS = [
    'PTIME', 'PSTATUS', 'DONOR_ID', 'WL_ID_CODE', 'LISTING_CTR_CODE',
    'register_date', 'waitlist_removal_date', 'tx_date', 'composite_deathdate',
    'waitlist_deathdate'
]
...
post_tx_death_mask = (~data['tx_date'].isna()) & (data['PSTATUS'] == 1) & (
    ~data['PTIME'].isna())
# case 2: not transplanted, dead in wait list
waitlist_death_mask = (data['tx_date'].isna()) & (
    ~data['waitlist_deathdate'].isna())
...
y = pd.DataFrame(index=survial_data.index,
...
X.to_hdf(f'{save_dir}/transplant_data_with_survival_time.h5',
         key='Patient',
         mode='w')
O.to_hdf(f'{save_dir}/transplant_data_with_survival_time.h5',
         key='Organ',
         mode='a')
y.to_hdf(f'{save_dir}/transplant_data_with_survival_time.h5',
         key='SurvivalTime',
         mode='a')
