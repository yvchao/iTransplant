...
with open(match_classification, 'rb') as dat_table:
    classification_data = pd.read_table(dat_table,
                                        names=columns,
                                        na_values=[' ', '.'],
                                        encoding='cp1252')
...
for audit_id in classification_data.ALLOC_AUDIT_ID.unique():
    classification_dict[audit_id] = classification_data[
        classification_data.ALLOC_AUDIT_ID == audit_id].drop(
            drop_list, axis=1).set_index('ID')
...
def create_allocation_type_lookup(
    ...
    def lookup_table(key_pairs):
        return [
            share_type(dictionary[key1].loc[key2].DESCRIP)
