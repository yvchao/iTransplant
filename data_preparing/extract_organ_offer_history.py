...
from selected_features import PATIENT_FEATURES, ORGAN_FEATURES, INITIAL_PATIENT_FEATURES, COLUMN_IS_CATEGORICAL
...
patient_data = pd.read_csv(f'{transplant_data_dir}/patient_data_cleaned.csv')[
    ['WL_ID_CODE', 'register_date'] + PATIENT_FEATURES +
...
with open(wl_data, 'rb') as dat_table:
    wl_history = pd.read_table(dat_table,
                               names=columns,
                               na_values=[' ', '.'],
                               encoding='cp1252')
...
used_columns = [
    'WL_ID_CODE', 'MELD_PELD_LAB_SCORE', 'BILIRUBIN', 'INR', 'SERUM_CREAT',
    'SERUM_SODIUM', 'ALBUMIN', 'DIALYSIS_PRIOR_WEEK', 'ENCEPH', 'ASCITES',
    'DONCRIT_MIN_AGE', 'DONCRIT_MAX_AGE', 'DONCRIT_MIN_WGT', 'DONCRIT_MAX_WGT',
    'DONCRIT_MAX_BMI'
]
...
def generate_offer_history(center_id_code,
                           wl_records,
                           save_dir,
                           feature_dtypes=feature_dtypes):
    offer_data = pd.read_hdf(f'{match_data_dir}/match_data.h5',
                             key=center_id_code,
                             mode='r')
    ...
    offer_data.loc[:,
                   'WLREG_AUDIT_ID_CODE'] = offer_data.WLREG_AUDIT_ID_CODE.astype(
                       int)
    ...
    offer_data = offer_data[offer_data.WLREG_AUDIT_ID_CODE.isin(
        wl_records.index)]
    print(
        f'records number after removal of unknown patients:{len(offer_data)}')

    # collect patient and organ data for considered organ offers
    selected_donor_data = donor_data.loc[offer_data.DONOR_ID].copy()
    selected_wl_data = wl_records.loc[offer_data.WLREG_AUDIT_ID_CODE,
                                      used_columns].copy()
    selected_patient_data = patient_data.loc[
        selected_wl_data.WL_ID_CODE].copy()
    ...
    delta = offer_data.MATCH_SUBMIT_DATE.values - selected_patient_data.register_date.values
    selected_patient_data.loc[:,
                              'AGE'] = selected_patient_data.INIT_AGE + delta.astype(
                                  'timedelta64[Y]').astype(int)
    ...
    replace_features = [('INR_TX', 'INR'),
                        ('FINAL_SERUM_SODIUM', 'SERUM_SODIUM'),
                        ('ALBUMIN_TX', 'ALBUMIN'),
                        ('DIAL_TX', 'DIALYSIS_PRIOR_WEEK'),
                        ('ALBUMIN_TX', 'ALBUMIN'), ('ASCITES_TX', 'ASCITES'),
                        ('TBILI_TX', 'BILIRUBIN')]

    for f in fill_features:
        mask = ~selected_wl_data[f].isna()
        selected_patient_data.loc[mask.to_list(),
                                  f] = selected_wl_data.loc[mask, f].values

    for f_, f in replace_features:
        mask = ~selected_wl_data[f].isna()
        selected_patient_data.loc[mask.to_list(),
                                  f_] = selected_wl_data.loc[mask, f].values
    ...
    for column in ['SHARE_TY']:
        selected_donor_data.loc[:,
                                column] = selected_donor_data[column].astype(
