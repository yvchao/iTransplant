...
print("entries after drop cases without WL_ID_CODE and INIT_DATE:%d" % len(liver))
...
liverdrop["coronary"] = np.where(
    (liverdrop.CORONARY_ANGIO_DON == "Y") & (liverdrop.CORONARY_ANGIO_NORM_DON == 0),
    1,
    liverdrop.coronary,
)

liverdrop["coronary"] = np.where(
    (liverdrop.CORONARY_ANGIO_DON == "Y") & (liverdrop.CORONARY_ANGIO_NORM_DON == "Y"),
    2,
    liverdrop.coronary,
)
...
liverdrop["macro"] = np.where(
    (liverdrop.LI_BIOPSY == "Y") & (liverdrop.MACRO_FAT_LI_DON < 30), 1, liverdrop.macro
)
...
liverdrop["micro"] = np.where(
    (liverdrop.LI_BIOPSY == "Y") & (liverdrop.MICRO_FAT_LI_DON < 30), 1, liverdrop.micro
)
...
liverdrop["composite_deathdate"] = pd.to_datetime(liverdrop["COMPOSITE_DEATH_DATE"])
