...
columns=['center id', 'offer number', 'acceptance rate', 'accepted offers', 'offer number (filtered)', 'acceptance rate (filtered)', 'accepted offers (filtered)']
stats=[]
for center_id_code in listing_centers:
    offer_data=pd.read_hdf(os.path.join(match_data_dir,'match_data.h5'),key=center_id_code,mode='r')
    offer_data=offer_data[offer_data.OFFER_ACCEPT!='Z']
    acceptance_rate=(offer_data.OFFER_ACCEPT=='Y').sum()/len(offer_data)

    filtered_offer_data=pd.read_hdf(os.path.join(offer_data_dir,f'{center_id_code[1:]}.h5'),key='Offer',mode='r')
    filtered_offer_data=filtered_offer_data[filtered_offer_data.OFFER_ACCEPT!='Z']

    filtered_acceptance_rate=(filtered_offer_data.OFFER_ACCEPT=='Y').sum()/len(filtered_offer_data)

    stats.append([center_id_code[1:], len(offer_data), acceptance_rate, (offer_data.OFFER_ACCEPT=='Y').sum(), len(filtered_offer_data), filtered_acceptance_rate, (filtered_offer_data.OFFER_ACCEPT=='Y').sum()])

stats=pd.DataFrame(stats, columns=columns)
stats=stats.sort_values('accepted offers (filtered)', ascending=False).reset_index(drop=True)
